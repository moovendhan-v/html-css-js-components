    ...
    try {
        ...
        createFiles(basePath, categories, folder_name, { html:html, css:css, js:js }, async (err) => {
            if (err) {
                ...
                res.send(sendJSONError({ errorStatus: true, statusCode: "400", message: `Data Not inserted Please contact admin plase visit contact us page : ${err}` }));
                return;
            } else {
                console.log('Files created successfully.'); 
                try {
                    await newComponents.save();
                    res.send(sendJSONSuccess({ errorStatus: false, statusCode: "201", message: 'Components has been updated Please wait for approval thank you for your contributons' }));
                } catch (error) {
                    console.error('Error saving new components:', error);
                    res.send(sendJSONError({ errorStatus: true, statusCode: "400", message: `Data Insert fails Please contact admin please visit contact us page : ${error}` }));
                }   
            } 
        });        
        ...
        res.send(sendJSONError({ errorStatus: true, statusCode: "400", message: `Data Insert fails : ${error}` }));
    }
}
